...
class MapManager {
        ...
        if CLLocationManager.locationServicesEnabled() {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) { //*1
                self.showAlert(
                    ...
                    message: "To enable it Go to: Settngs -> Privacy - Location Services an turn On"
                )
            }
        }
    ...
    func checkLocationAuthorization(mapView: MKMapView, segueIdentifier: String) {
        ...
        case .denied:
            DispatchQueue.main.asyncAfter(deadline: .now() + 1){ //*1
                self.showAlert(
                    title: "Your Location is not Avaliable",
    ...
    //Построение маршрута от местоположения пользователя до метки места
    func getDirections(for mapView: MKMapView, previousLocation: (CLLocation) -> ()) {
        ...
        //отслеживание текущего местоположения (активируем только после того, как убедимся что отслеживание вообще включено)
        ...
        resetMapView(withNew: directions, mapView: mapView)   //для перестроения маршрутов
    ...
    } //для реализации отображения смотри строки с комментарием *1
}
